...
void Level5::Initialize()
{
	...
	const int width = level5.GetWidth();
	const int height = level5.GetHeight();

	State::sCurrentState = this;

	for (int i = 0; i < width; i++)
	{
		level5.SetIndex(i, 0, Index::WALL);
		level5.SetIndex(i, height - 1, Index::WALL);
	}
	for (int i = 0; i < height; i++)
	{
		level5.SetIndex(0, i, Index::WALL);
		level5.SetIndex(width - 1, i, Index::WALL);
	}
	for (int i = 2; i <= 5; i++)
	{
		level5.SetIndex(i, 6, Index::WALL);
		level5.SetIndex(i, 5, Index::WALL);
	}
	level5.SetIndex(1, 1, Index::WALL);
	level5.SetIndex(2, 1, Index::WALL);
	level5.SetIndex(4, 1, Index::WALL);
	level5.SetIndex(5, 1, Index::WALL);
	level5.SetIndex(5, 2, Index::WALL);

    Player1.SetCharacterXIndex(5);
    Player1.SetCharacterYIndex(4);
    Player2.SetCharacterXIndex(5);
    Player2.SetCharacterYIndex(3);
    level5.SetIndex(Player1.GetCharacterXIndex(), Player1.GetCharacterYIndex(), Index::MAG);
    level5.SetIndex(Player2.GetCharacterXIndex(), Player2.GetCharacterYIndex(), Index::NET);
    Player1.SetMovableStatus(Character::Unable);
    Player2.SetMovableStatus(Character::Unable);

    level5.InitializeMapTile(width, height);

	level5.SetIndex(1, 2, Index::BOX);
	level5.SetIndex(1, 4, Index::BOX);
	level5.SetIndex(2, 2, Index::BOX);
	level5.SetIndex(2, 3, Index::BOX);
	level5.SetIndex(2, 4, Index::BOX);
	level5.SetIndex(3, 2, Index::BOX);
    ...
    level5.SetIndex(player1Clear.GetClearMagXIndex(), player1Clear.GetClearMagYIndex(), Index::CLEAR_MAG);
    level5.SetIndex(player2Clear.GetClearNetXIndex(), player2Clear.GetClearNetYIndex(), Index::CLEAR_NET);
	...
	BoxMove->Level = &level5;
	BoxMove1->Level = &level5;

	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			switch (level5.GetIndex(x, y))
			{
...
void Level5::Update(float dt)
{
	...
	if ((level5.GetIndex(Player1.GetCharacterXIndex(), Player1.GetCharacterYIndex()) ==
		level5.GetIndex(player1Clear.GetClearMagXIndex(), player1Clear.GetClearMagYIndex())) &&
		(level5.GetIndex(Player2.GetCharacterXIndex(), Player2.GetCharacterYIndex()) ==
			level5.GetIndex(player2Clear.GetClearNetXIndex(), player2Clear.GetClearNetYIndex())))
	{
		m_game->SetLevelId(5);
